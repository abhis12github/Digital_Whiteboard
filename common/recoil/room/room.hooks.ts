...
export const useSetUsers = () => {
  ...
  const handleAddUser = (userId: string, name: string) => {
    setRoom((prev) => {
      ...
      // Get the color assigned to the last user and to the new user assign the next color in the list.
      // This color is just for identifying different users and not canvas color.
